...
function includesToday(daysOfWeek: number[], now: Date) {
  return daysOfWeek.includes(now.getUTCDay());
}

function matchesMinute(localTime: string, now: Date) {
  const hh = String(now.getUTCHours()).padStart(2, '0');
  const mm = String(now.getUTCMinutes()).padStart(2, '0');
  return localTime === `${hh}:${mm}`;
}

function buildRunKey(scheduleId: string, now: Date) {
...
export async function runSchedulerTick(workspaceId: string) {
  const now = new Date();
  ...
  for (const schedule of schedules) {
    ...
    if (!includesToday(schedule.daysOfWeek, now)) continue;
    if (!matchesMinute(schedule.localTime, now)) continue;
