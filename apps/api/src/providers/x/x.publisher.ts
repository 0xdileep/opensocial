...
async function uploadImageToX(token: string, imageUrl: string) {
  ...
  if (!initRes.ok) throw new AppError(`X media initialize failed with ${initRes.status}`);

  const initJson = await initRes.json() as { data?: { id: string } };
  const mediaId = initJson.data?.id;
  if (!mediaId) throw new AppError('X media initialize did not return media id');
  ...
  if (!appendRes.ok) throw new AppError(`X media append failed with ${appendRes.status}`);
  ...
  if (!finalizeRes.ok) throw new AppError(`X media finalize failed with ${finalizeRes.status}`);
...
export async function publishToX(input: XPublishInput) {
  ...
  if (!input.imageUrl) {
    ...
    if (!response.ok) throw new AppError(`X tweet publish failed with ${response.status}`);
