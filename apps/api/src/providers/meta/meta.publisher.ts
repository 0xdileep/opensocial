...
async function publishFacebookPost(input: MetaPublishInput) {
  const pageId = typeof input.account.meta?.pageId === 'string' ? input.account.meta.pageId : null;
  if (!pageId) throw new AppError('Meta Facebook account requires meta.pageId');
...
async function getInstagramContainerStatus(containerId: string, token: string) {
  return jsonRequest<{ status_code?: string }>(`https://graph.facebook.com/v20.0/${containerId}?fields=status_code`, {
    method: 'GET',
    headers: { Authorization: `Bearer ${token}` }
  });
}

async function pollInstagramContainer(containerId: string, token: string) {
  ...
  for (let attempt = 0; attempt < 8; attempt += 1) {
    ...
    if (lastStatus === 'ERROR' || lastStatus === 'EXPIRED') throw new AppError(`Instagram media container failed with status ${lastStatus}`);
    await new Promise((resolve) => setTimeout(resolve, 1500));
  }
  throw new AppError(`Instagram media container did not finish in time: ${lastStatus}`, 504);
}

async function publishInstagramPost(input: MetaPublishInput) {
  ...
  if (!instagramBusinessId) throw new AppError('Meta Instagram account requires meta.instagramBusinessId');
  if (!input.imageUrl) throw new AppError('Instagram publishing requires imageUrl in this implementation');

  await assertRemoteImageUrl(input.imageUrl);

  const creation = await jsonRequest<{ id?: string }>(`https://graph.facebook.com/v20.0/${instagramBusinessId}/media`, {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${input.account.accessToken}`,
      'Content-Type': 'application/json'
  ...
  if (!creation.id) throw new AppError('Instagram media creation did not return container id');
...
export async function publishToMeta(input: MetaPublishInput) {
  if (input.account.platform === 'meta_facebook') return publishFacebookPost(input);
