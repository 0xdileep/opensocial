...
import { ScheduleRule } from '@social-ai/shared';
import { query } from '../../db/client.js';

export async function createSchedule(input: Omit<ScheduleRule, 'id' | 'lastRunAt'>) {
  const id = randomUUID();
  const result = await query<ScheduleRule>(
