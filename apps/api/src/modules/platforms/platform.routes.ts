...
import { z } from 'zod';
import { createPlatformAccount, listPlatformAccounts, PlatformAccount } from './platform.service.js';
...
export async function platformRoutes(app: FastifyInstance) {
  app.get('/v1/platforms/accounts', async (request) => {
    ...
    if (resolvedWorkspaceId !== workspaceId) throw new AppError('Workspace mismatch', 403);
    const rows = await listPlatformAccounts(resolvedWorkspaceId);
    return rows.map((row: PlatformAccount) => ({
      ...row,
