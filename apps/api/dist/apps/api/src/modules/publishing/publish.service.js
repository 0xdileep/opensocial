...
async function publishOne(postId, workspaceId, platform, text, imageUrl) {
    const account = await getPlatformAccount(workspaceId, platform);
    if (!account)
        throw new AppError(`No connected account for ${platform}`, 400);
...
export async function publishPostNow(postId, workspaceId) {
