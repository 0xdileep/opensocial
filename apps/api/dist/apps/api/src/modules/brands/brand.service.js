...
export async function createBrand(input) {
    const id = randomUUID();
    const result = await query(`insert into brand_profiles (
...
export async function getBrandById(id, workspaceId) {
    const result = workspaceId
        ? await query(`select
          ...
          created_at as "createdAt",
          updated_at as "updatedAt"
         from brand_profiles where id = $1 and workspace_id = $2`, [id, workspaceId])
        : await query(`select
          id,
          workspace_id as "workspaceId",
          name,
