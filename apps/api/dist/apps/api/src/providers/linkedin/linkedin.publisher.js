...
async function getPostLifecycleState(token, ugcPostUrn) {
...
export async function publishToLinkedIn(input) {
    ...
    if (!authorUrn)
        throw new AppError('LinkedIn account requires meta.authorUrn');
    if (!input.imageUrl) {
    ...
    const register = await jsonRequest('https://api.linkedin.com/v2/assets?action=registerUpload', {
        method: 'POST',
        headers: baseHeaders(input.account.accessToken, { 'Content-Type': 'application/json' }),
        body: JSON.stringify({
    ...
    if (!upload.ok)
        throw new AppError(`LinkedIn media upload failed with ${upload.status}`);
    const publishResult = await jsonRequest('https://api.linkedin.com/v2/ugcPosts', {
        method: 'POST',
        headers: baseHeaders(input.account.accessToken, { 'Content-Type': 'application/json' }),
        body: JSON.stringify({
            author: authorUrn,
