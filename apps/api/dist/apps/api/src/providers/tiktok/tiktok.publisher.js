...
function getCreatorInfo(account) {
    const openId = typeof account.meta?.openId === 'string' ? account.meta.openId : null;
    if (!openId)
        throw new AppError('TikTok account meta.openId is required');
    return { openId };
}
function getVideoUrl(account) {
    const videoUrl = typeof account.meta?.videoUrl === 'string' ? account.meta.videoUrl : null;
    if (!videoUrl)
        throw new AppError('TikTok requires meta.videoUrl for video publishing');
    return videoUrl;
}
function getPhotoUrls(account, explicitImageUrl) {
    const metaPhotoImages = Array.isArray(account.meta?.photoImages)
        ? account.meta.photoImages.filter((item) => typeof item === 'string' && item.length > 0)
        : [];
    const urls = explicitImageUrl ? [explicitImageUrl, ...metaPhotoImages] : metaPhotoImages;
    const unique = [...new Set(urls)];
    if (unique.length === 0)
        throw new AppError('TikTok photo posting requires imageUrl or meta.photoImages');
    return unique;
}
function getPrivacyLevel(account) {
    ...
    if (!allowed.has(privacyLevel))
        throw new AppError('Invalid TikTok privacyLevel in meta');
    return privacyLevel;
}
function getPostMode(account) {
    ...
    if (!allowed.has(postMode))
        throw new AppError('Invalid TikTok postMode in meta');
    return postMode;
}
async function initVideoUpload(token, openId, caption, privacyLevel) {
    const payload = {
        post_info: {
    ...
    const result = await jsonRequest('https://open.tiktokapis.com/v2/post/publish/video/init/', {
        method: 'POST',
        headers: {
    ...
    if (result.error?.message)
        throw new AppError(`TikTok init failed: ${result.error.message}`);
    const publishId = result.data?.publish_id;
    const uploadUrl = result.data?.upload_url;
    if (!publishId || !uploadUrl)
        throw new AppError('TikTok init did not return publish_id and upload_url');
    return { publishId, uploadUrl };
}
async function initPhotoPost(token, caption, photoImages, postMode) {
    const payload = {
        post_info: {
            title: caption.slice(0, 90),
            description: caption
    ...
    const result = await jsonRequest('https://open.tiktokapis.com/v2/post/publish/content/init/', {
        method: 'POST',
        headers: {
            Authorization: `Bearer ${token}`,
            'Content-Type': 'application/json'
    ...
    if (result.error?.message)
        throw new AppError(`TikTok photo init failed: ${result.error.message}`);
    const publishId = result.data?.publish_id;
    if (!publishId)
        throw new AppError('TikTok photo init did not return publish_id');
    return { publishId };
}
async function uploadVideoFromRemoteUrl(uploadUrl, remoteUrl) {
    const mediaResponse = await fetch(remoteUrl);
    if (!mediaResponse.ok)
        throw new AppError(`Could not fetch remote video: ${mediaResponse.status}`);
    const contentType = mediaResponse.headers.get('content-type') || 'video/mp4';
    if (!contentType.includes('video'))
        throw new AppError('TikTok remote asset is not a video content-type');
...
async function getPublishStatus(token, publishId) {
    const result = await jsonRequest('https://open.tiktokapis.com/v2/post/publish/status/fetch/', {
        method: 'POST',
        headers: {
            Authorization: `Bearer ${token}`,
            'Content-Type': 'application/json'
    ...
    if (result.error?.message)
        throw new AppError(`TikTok status fetch failed: ${result.error.message}`);
    return result.data ?? {};
}
async function pollStatus(token, publishId) {
    for (let attempt = 0; attempt < 5; attempt += 1) {
        ...
        if (state === 'PUBLISH_COMPLETE' || state === 'SEND_TO_USER_INBOX')
            return status;
        if (state === 'FAILED')
            throw new AppError(`TikTok publish failed: ${status.fail_reason || 'Unknown failure'}`);
